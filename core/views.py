...
def registro(request):
    if request.method == 'POST':
        formUser = RegistroUsuarioForm(request.POST)
        formPerfil = RegistroPerfilForm(request.POST, request.FILES)
    ...
    if request.method == 'GET':
        ...
        formUser = RegistroUsuarioForm()
        formPerfil = RegistroPerfilForm()
...
def misdatos(request):
    ...
    if request.method == 'POST':
        formUser = UsuarioForm(request.POST, instance=request.user)
        formPerfil = RegistroPerfilForm(request.POST, request.FILES, instance=request.user.perfil)   

        print(formUser.is_valid(), formPerfil.is_valid()) 
        if formUser.is_valid() and formPerfil.is_valid():
            user = formUser.save(commit=False)
            user.is_staff = False
            perfil = formPerfil.save(commit=False)
    ...
    if request.method == 'GET':
        formUser = UsuarioForm(instance=request.user)
        formPerfil = RegistroPerfilForm(instance=request.user.perfil)
...
def boleta(request, nro_boleta):
    ...
    if Boleta.objects.filter(nro_boleta=nro_boleta).exists():
        ...
        boletaUser = Boleta.objects.filter(nro_boleta=nro_boleta, cliente=request.user.perfil)
        if boletaUser or request.user.is_staff:
            detalle_boleta = DetalleBoleta.objects.filter(boleta=boleta)
        else:
            messages.error(request, f'Lo sentimos, la boleta N|{nro_boleta} pertenece a {boleta.cliente}')
            boleta = None
...
def miscompras(request):
    ...
    historial = Boleta.objects.filter(cliente=perfil)
